...
<script lang="ts" setup>
  ...
  function alertHandler(data: AlertTimeout): void {
    ...
    setTimeout(() => {
      const prevIndex = alerts.value.length - 2;
      if (alerts.value[prevIndex] && !alerts.value[prevIndex].playing) {
        alerts.value[prevIndex].playing = true;
        if (gdComponent.value) {
          animate.remove(gdComponent.value, alerts.value[prevIndex].id, () => {
            if (alerts.value[prevIndex]?.timeout)
              clearTimeout(alerts.value[prevIndex].timeout);
